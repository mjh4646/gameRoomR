...
getLook = function(gameState, obj){
	...
	if(obj == "FirstRoom"){
        ...
        if(gameState$Switch==FALSE){
            ...
            plot(gameState$Means*100, main="1,000 sets of coins", xlab="Set", ylab="Percentage of heads",ylim=c(0,100),cex=0.7)
            abline(h=47, lty=3)
            abline(h=53, lty=3)
            print(paste("The screen shows the percentage of heads for 1,000 sets of tosses of", gameState$coins, "coins."))
        }
	}
...
toss = function(coins){
    ...
    if(tempState$Switch == FALSE){
        ...
        print(paste("You pushed the button.",coins,"coins are tossed, of which",sum(tempState$Data),"are heads."))
    } else if(tempState$Switch == TRUE){
        tempState$Coins = coins
        tempState$Means = rbinom(1000,coins,0.5)/coins
        print(paste("You pushed the button. One thousand sets of",coins,"coins are tossed. The number of heads ranges from",min(tempState$Means*coins),"to",max(tempState$Means*coins)))
    }
...
useInterpreter = function(gameState,obj){
	...
	if(obj == "Button"){
        ...
        tempState = get("gameState", myE)
        if(tempState$Switch==FALSE){
            print("None of the dots are within the dotted lines. Perhaps you should use the switch?")
        } else {
            prop = mean(tempState$Means <= 0.53) - mean(tempState$Means <= 0.47)
            if(prop >= 0.935 && prop <= 0.965){
                print(paste(100*prop,"percent of the dots are within the dotted lines. The door opens and you escape into the outside world. You win! (trumpets)"))
            } else {
                print(paste(100*prop,"percent of the dots are within the dotted lines. You need 95%. Keep trying!"))
            }
        }
	} else if(is.null(obj)){
